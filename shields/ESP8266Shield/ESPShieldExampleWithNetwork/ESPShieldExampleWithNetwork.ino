...
char ssid[] = "NODE_GROUND";            // your network SSID (name)
char pass[] = "IAmAMeshNetworkPassword";        // your network password
...
const int UDP_TIMEOUT = 20000;    // timeout in miliseconds to wait for an UDP packet to arrive
...
// if port on the esp is set to 115200 
// you can set it permanent to 9600 with the following code  

// swser.begin(115200);
// Serial.println(swser.readString()); 
// switch to 9600 Boud its more save for software serial
// swser.print("AT+UART_DEF=9600,8,1,0,0\r\n");
// delay (1000);

 swser.begin(9600);
 
  swser.print("AT+UART_CUR?\r\n");
  while(swser.available() < 1);
 Serial.println(swser.readString()); 
 
 
  swser.print("AT+CIPSTAMAC_CUR?\r\n");
  while(swser.available() < 1);
  Serial.println(swser.readString()); 
